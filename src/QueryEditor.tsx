...
function filterOptions(option:SelectableValue, search:string) {
  if(option.value == search) { return(true); }
  try {
...
export const QueryEditor = (props: Props) => {
  ...
  const prependDasboardVariables = (data:SelectableValue[]) => {
    getTemplateSrv().getVariables().forEach((v, i) => {
      data.unshift({
        label: '/^$'+v.name+'$/',
        value: '/^$'+v.name+'$/',
      });
  ...
  const loadHosts = (filter: string): Promise<SelectableValue[]> => {
    ...
    return(lastValueFrom(props.datasource.request("GET", "/index.php/api/hosts"))
      .then(response => {
        return(response.data.hosts.map(
            (row:{name?:string}) => { return({ label: row.name, value: row.name}); }
          ));
      })
  ...
  const loadServices = (filter: string): Promise<SelectableValue[]> => {
    return(lastValueFrom(props.datasource.request("POST", "/index.php/api/services", { host: props.query.host || '/.*/' }))
      .then(response => {
        return(response.data.services.map(
            (row:{name?:string,servicedesc?:string}) => { return({ label: row.servicedesc || row.name, value: row.name}); }
          ));
      })
      .then(prependDasboardVariables)
    );
  ...
  const loadLabel = (filter: string): Promise<SelectableValue[]> => {
    return(lastValueFrom(props.datasource.request("POST", "/index.php/api/labels", { host: props.query.host || '/.*/', service: props.query.service || '/.*/' }))
      .then(response => {
        return(response.data.labels.map(
            (row:{name?:string,label?:string}) => { return({ label: row.label || row.name, value: row.label || row.name}); }
          ));
      })
      .then(prependDasboardVariables)
    );
  ...
  return (
    ...
    <div className="gf-form">
      <SegmentSection label="Select" fill={false}>
        <InlineSegmentGroup grow={true}>
          <InlineLabel htmlFor="" width={6} className="">Host:</InlineLabel>
          <div className={selectClass}>
            <AsyncSelect
              defaultOptions
              value={toSelectableValue(props.query.host || '')}
              loadOptions={loadHosts}
              onChange={v => { onValueChange('host', v.value)}}
        ...
        <InlineSegmentGroup grow={true}>
          <InlineLabel htmlFor="" width="auto" className="">Service:</InlineLabel>
          <div className={selectClass}>
            <AsyncSelect
              ...
              onChange={v => { onValueChange('service', v.value)}}
        ...
        <InlineSegmentGroup grow={true}>
          <InlineLabel htmlFor="" width="auto" className="">Label:</InlineLabel>
          <div className={selectClass}>
            <AsyncSelect
              ...
              onChange={v => { onValueChange('perflabel', v.value)}}
        ...
        <InlineSegmentGroup grow={true}>
          <InlineLabel htmlFor="" width="auto" className="">Type:</InlineLabel>
          <div className="">
            <Select
              options={['AVERAGE', 'MIN', 'MAX', 'WARNING', 'CRITICAL'].map(toSelectableValue)}
              onChange={v => { onValueChange('type', v.value)}}
    ...
    <div className="gf-form">
      <SegmentSection label="Options" fill={false}>
        <InlineSegmentGroup grow={true}>
          <InlineLabel htmlFor="" width={6} className="">Fill:</InlineLabel>
          <div className="">
            <Select
              options={['fill', 'zero', 'gap'].map(toSelectableValue)}
              onChange={v => { onValueChange('fill', v.value)}}
        ...
        <InlineSegmentGroup grow={true}>
          <InlineLabel htmlFor="" width={8} className="">Factor:</InlineLabel>
          <div className="">
            <Input
              defaultValue={""}
              width={36}
              onChange={v => { onValueChange('factor', v.currentTarget.value)}}
              label="Factor:"
        ...
        <InlineSegmentGroup grow={true}>
          <div className="">
            <Input
              defaultValue={""}
              width={60}
              onChange={v => { onValueChange('alias', v.currentTarget.value)}}
